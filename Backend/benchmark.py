"""
DocMind RAGAS Benchmark Runner v4 — Chunker Comparison Edition
==================================================================
Same metrics and optimizations as v3, with two changes for comparing
chunking strategies (naive / hierarchical / semantic / sentence):

  1. NO ARTIFICIAL RATE-LIMIT DELAYS — assumes a paid Gemini tier (no more
     free-tier 5 req/min throttling). Retry-on-error logic is kept for
     genuine transient failures, but the deliberate time.sleep(15) between
     every call is gone, so runs are much faster.

  2. --chunker LABEL — results are organized into:
         benchmark_results/<chunker_label>/query_<timestamp>.csv
         benchmark_results/<chunker_label>/compare_<timestamp>.csv
         benchmark_results/<chunker_label>/summary_<timestamp>.json
     Run the same command once per chunking strategy (after re-uploading
     docs with that chunker in DocMind) and each run's results land in
     their own folder — no manual renaming needed.

     Example:
       python ragas_benchmark_v4.py --chunker naive
       python ragas_benchmark_v4.py --chunker hierarchical
       python ragas_benchmark_v4.py --chunker semantic
       python ragas_benchmark_v4.py --chunker sentence

     The score cache is ALSO namespaced per chunker (benchmark_results/<chunker>/score_cache.json)
     since different chunkers produce different contexts and shouldn't share cache entries.

Metrics (unchanged from v3):
  Faithfulness        — Gemini (needs real reasoning about claim support)
  Context Recall      — Gemini (needs real reasoning about ground truth coverage)
  Answer Relevancy    — local SentenceTransformer cosine similarity
  Context Precision   — local SentenceTransformer cosine similarity (or --use-gemini-precision)

  3. --auto-upload + --test-data-dir — fully automates the upload step:
         python ragas_benchmark_v4.py --chunker naive --auto-upload
     This will:
       a. Delete every document currently in DocMind (skip with --no-wipe)
       b. Upload every .pdf found in ./Test_Data/ (override with --test-data-dir)
       c. Run the full /query + /compare benchmark
       d. Save results to benchmark_results/naive/

     THE ONE MANUAL STEP: chunking strategy is selected by which function
     upload.py calls (naive_chunk / hierarchical_chunk / semantic_chunk /
     sentence_chunk), so you still need to edit that import/call in
     app/routes/upload.py and restart your uvicorn server before each run.
     Everything else — wiping old docs, uploading the test set, running
     every benchmark question, scoring, saving — is automatic.

     Full 4-chunker workflow:
       1. Edit upload.py to use naive_chunk, restart server
       2. python ragas_benchmark_v4.py --chunker naive --auto-upload
       3. Edit upload.py to use hierarchical_chunk, restart server
       4. python ragas_benchmark_v4.py --chunker hierarchical --auto-upload
       5. Repeat for semantic_chunk, sentence_chunk
       6. python ragas_benchmark_v4.py --compare-chunkers

Usage:
  python ragas_benchmark_v4.py --chunker naive --auto-upload --api-key YOUR_KEY
  python ragas_benchmark_v4.py --chunker semantic --auto-upload --test-data-dir ./Test_Data
  python ragas_benchmark_v4.py --chunker sentence --two-stage --top-n 5
  python ragas_benchmark_v4.py --chunker hierarchical --auto-upload --clear-cache
